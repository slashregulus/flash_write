...
static void pll3_control(uint32_t high)
{
	...
	if((Prr_Product==PRR_PRODUCT_M3)||((Prr_Product==PRR_PRODUCT_H3)&&(Prr_Cut<=PRR_PRODUCT_20))){
...
static void ddrtbl_load(void)
{
	...
	***********************************************************************/
	...
	for(i=0;i<2;i++){
		for(csab=0;csab<CSAB_CNT;csab++){
			ddrtbl_setval(_cnf_DDR_PI_REGSET, _reg_PI_MR11_DATA_Fx_CSx[i][csab], _def_LPDDR4_ODT);
		}
	}
	...
	for(i=0;i<2;i++){
		for(csab=0;csab<CSAB_CNT;csab++){
			ddrtbl_setval(_cnf_DDR_PI_REGSET, _reg_PI_MR12_DATA_Fx_CSx[i][csab], _def_LPDDR4_VREFCA);
		}
	}
	...
	***********************************************************************/
	...
	if(((Prr_Product==PRR_PRODUCT_M3)||(Prr_Product==PRR_PRODUCT_M3N))&&
	   ((0x00ffffff & (uint32_t)((Boardcnf->ch[0].ca_swap)>>40)) != 0x00)){
		...
		for(i=0;i<2;i++){
			for(csab=0;csab<CSAB_CNT;csab++){
				ddrtbl_setval(_cnf_DDR_PI_REGSET, _reg_PI_MR11_DATA_Fx_CSx[i][csab], 0x66);
			}
		}
#endif//_def_LPDDR4_ODT
	} else {
	...
	***********************************************************************/
		...
		if(ddr_phycaslice==1){
			...
			if(Prr_Product==PRR_PRODUCT_M3){
				...
				ddr_setval_s(ch, 2, _reg_PHY_ADR_CALVL_SWIZZLE1_0, 0x00000000);
				ddr_setval_s(ch, 2, _reg_PHY_ADR_CALVL_SWIZZLE0_1, dataL);
				ddr_setval_s(ch, 2, _reg_PHY_ADR_CALVL_SWIZZLE1_1, 0x00000000);
				ddr_setval_s(ch, 2, _reg_PHY_ADR_CALVL_DEVICE_MAP, _par_CALVL_DEVICE_MAP);
			} else {
				ddr_setval_s(ch, 2, _reg_PHY_ADR_CALVL_SWIZZLE0, dataL);
				ddr_setval_s(ch, 2, _reg_PHY_ADR_CALVL_SWIZZLE1, 0x00000000);
				ddr_setval_s(ch, 2, _reg_PHY_CALVL_DEVICE_MAP, _par_CALVL_DEVICE_MAP);
			}
	...
	for(ddr_csn=0;ddr_csn<CS_CNT;ddr_csn++){
		...
		foreach_vch(ch){
			if(ch_have_this_cs[ddr_csn] & (1U<<ch)){
				for(slice=0;slice<SLICE_CNT;slice++){
					if(ddr_getval_s(ch, slice, _reg_PHY_RDLVL_STATUS_OBS)!=0x0D00FFFF){
...
void pvtcode_update(void)
{
	...
	foreach_vch(ch){
		...
		if((Prr_Product==PRR_PRODUCT_H3)&&(Prr_Cut<=PRR_PRODUCT_11)){
			...
			reg_ddrphy_write(ch, ddr_regdef_adr(_reg_PHY_PAD_FDBK_TERM), dataL | 0x00020000);
			reg_ddrphy_write(ch, ddr_regdef_adr(_reg_PHY_PAD_DATA_TERM), dataL);
			reg_ddrphy_write(ch, ddr_regdef_adr(_reg_PHY_PAD_DQS_TERM ), dataL);
			reg_ddrphy_write(ch, ddr_regdef_adr(_reg_PHY_PAD_ADDR_TERM), dataL);
			reg_ddrphy_write(ch, ddr_regdef_adr(_reg_PHY_PAD_CS_TERM  ), dataL);
		} else {
